<template>
  ...
  >
    ...
    <v-select
      ...
      v-model="model"
      :placeholder="__('Start typing to search for an user')"
    ...
    >
      <template v-slot:no-options="{ search, searching }">
        ...
        <em v-else style="opacity: 0.5"
          >{{ __("Start typing to search for an user") }}.</em
        >
      </template>
...
export default {
  props: ["modelValue", "options", "label", "name", "required", "bottom_margin", "multiple"],
  ...
  methods: {
    ...
    search: debounce((loading, search, vm) => {
      fetch(route("admin.users.searchAll") + `?q=${encodeURI(search)}`).then((res) => {
