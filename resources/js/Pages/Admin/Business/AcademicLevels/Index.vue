<template>
  <BusinessLayout :title="data.title">
    <TitleBar
      :title="data.title"
      :create_link="route('admin.academicLevels.create', { service: data.service.slug })"
      :previous_link="route('admin.services.configurationHome', data.service.slug)"
      :previous_link_text="__('Back to configuration')"
    />
    <SearchBar
      :url="route('admin.academicLevels.index', { service: data.service.slug })"
    ...
    <Table
      ...
      :links="academic_levels.links"
      :total="academic_levels.total"
    >
      <template v-slot>
        <tr v-for="(academic_level, index) in academic_levels.data" :key="index">
          <td>
            <Link
              :href="
                route('admin.academicLevels.edit', {
                  service: data.service.slug,
                  academicLevel: academic_level.slug,
                })
              "
              >{{ academic_level.name }}</Link
            >
          </td>
          <td class="text-end">
          ...
          <td class="col-md-2 text-end">
            <DestroyButton
              :delete_url="
                route('admin.academicLevels.destroy', {
                  service: data.service.slug,
                  academicLevel: academic_level.slug,
                })
              "
