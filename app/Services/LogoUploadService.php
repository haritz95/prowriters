...
class LogoUploadService
{
    ...
    function upload_logo_mobile(Request $request)
    {
        try {

            $old_company_logo_mobile = Setting::get_setting('company_logo_mobile');

            $file = $this->StoreBase64ToFile($request->input('file'));

            Setting::save_settings(['company_logo_mobile' => $file]);

            if (Storage::disk('public')->exists($old_company_logo_mobile)) {

                Storage::disk('public')->delete($old_company_logo_mobile);
            }
