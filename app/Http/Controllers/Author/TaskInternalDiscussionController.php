...
class TaskInternalDiscussionController extends Controller
{
    ...
    public function index(Request $request, Task $task)
    {
        ...
        return inertia('Author/Tasks/Discussions/Index', [
            ...
            'data' => [
                'title'      => __('app.author.internal_discussions.title'),
                'sub_title'  => __('app.author.internal_discussions.sub_title'),
                'user_types' => UserType::get(),
                'urls' => [
                    'new_message' => route('author.tasks.internal-discussions.create', $task->uuid),
                ],
            ],
        ]);
    }
    ...
    public function create(Task $task)
    {
        ...
        return Inertia::modal('Author/Tasks/Discussions/Create', [
            'task' => $task,
            'data' => [
                'config' => $this->taskMessageService->getConfigForCreateMessage(route('author.tasks.internal-discussions.store', $task->uuid)),
                'urls' => [
                    'message_list' => route('author.tasks.internal-discussions.index', $task->uuid),
                ]
            ]
        ])->baseRoute('author.tasks.internal-discussions.index', $task->uuid);
    }
    ...
    function store(StoreTaskMessageRequest $request, Task $task)
    {
        ...
        return redirect()->route('author.tasks.internal-discussions.index', $task->uuid)->withSuccess(__('Message sent successfully'));
    }
}
